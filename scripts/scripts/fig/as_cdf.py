"""Run at `scripts/` with `python3 -m scripts.fig.as_cdf`.
Data are from here:
...
def plot():
    ...
    df_sum = df[["aut_num"]].copy()
    df_percentages = df[["aut_num"]].copy()
    for tag in TAGS:
    ...
    ax.set_xlabel("Number of Routes by Controlling AS Pairs", fontsize=16)
    ...
    ax.set_xlabel("Percentages of Routes by Controlling AS Pairs", fontsize=16)
    ax.set_ylabel("Cumulative Fraction of AS Pairs", fontsize=16)
    ax.tick_params(axis="both", labelsize=14)
    ax.grid()
    ax.legend(loc="best", fontsize=14)
